...
def onset_metrics(words, label_text, time_map, lead_ms=0):
    ...
    for i, fields in enumerate(labels):
        if len(fields) != 3 or fields[2].strip() != words[i]['text'].strip():
        ...
        previous = truth
        deltas.append(words[i]['t'] - lead_ms - truth)
...
def main():
    ...
    if args.labels:
        ...
        result['onsets'] = onset_metrics(probe['words'], args.labels.read_text(encoding='utf-8'), meta['map'], args.lead_ms)
